...
#Previously: c("klima", "klimatický"). NB: that code includes all the "climate inside museums" or "societal climate" occurrences, too, and it's still small.
# To include bi- or trigrams, the line summarize(text = str_flatten(lemma, collapse = " ", na.rm = TRUE)) was needed.
...
process_df <- function(chunk_path) {
  chunk_path %>% 
    ...
    summarize(text = str_flatten(lemma, collapse = " ", na.rm = TRUE)) %>% 
        filter(lemma %in% terms) #This line now added.
}
...
list_of_full_chunks <- list.files(path = "~/Rprojekt/Climate/1.data_extraction/data/full_articles/",
                             pattern = "*.rds",
                             full.names = TRUE)
...
#Or, if already loaded: climate_terms  %>%  distinct()
...
# Frequency of selected lemmata by month - previous code only works for single lemmata, monograms, and then it created single lemmata.
#frequency_summary_climate_terms <- climate_terms %>% inner_join(doc_id_by_date, by = "doc_id") %>% mutate(month = as.Date(cut(date, "months"))) %>%  count(month, lemma, name = "monthly_count")
frequency_summary_climate_terms <- climate_terms %>% 
  inner_join(doc_id_by_date, by = "doc_id") %>%
  mutate(month = as.Date(cut(date, "months"))) %>%
  #separate_rows(text, sep = " ") %>%
  #rename(lemma = text) %>%
  count(month, name = "monthly_count") #When only counting single lemmata, add "lemma" as a weight for the count: count(month, lemma, name="XX").
...
ggplot(frequency_summary_climate_terms, aes(x = month, y = monthly_count, color = lemma)) +
  ...
  labs(title = "A decade of Czech Television news",
       subtitle = "Total monthly counts of documents containing at least one of the lemmatized terms, 2012-2022",
       caption = "Source: Newton Media Archive, January 2012 to April 2022. N=531,592.") +
  theme_bw() +
...
```{r}
documents_count <- data.frame(variable = names(table(climate_terms$lemma)), count_1000 = round(as.numeric(table(climate_terms$lemma)/1000), 1)) %>%
...
```
```{r New total climate document count}
#Code to extract the doc_id for the plot in chunk 8 did not work. Replacement: 
doc_id_list <- list()
