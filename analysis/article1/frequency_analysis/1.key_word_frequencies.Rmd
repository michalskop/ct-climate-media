...
# It filters the text by parts of speech (ADJ, NOUN, ADV, PRON, VERB), converts all lemmas to lowercase, and joins the data with a document ID by date. 
...
get_term_counts <- function(chunk_path) {
  text_stats <- chunk_path %>%
    readRDS() %>%
    filter(upos %in% c("ADJ", "NOUN", "ADV", "PRON", "VERB")) %>%
...
climate_terms <- c("změna klima\\b", "\\bklima\\b\\s+se\\s+měnit\\b", "\\bměnící\\s+se\\s+klima", "\\bglobální oteplování", "\\boteplování planeta", "klimatický\\b(?= změna|podmínka|dopad|důsledek|model|opatření|krize|kolaps|katastrofa|neutrální|neutralita|plán|rozvrat|zákon|závazek|žaloba|vzdělávání)")
...
```
```{r Counting the final string of terms for the Climate corpus v4}
climate_terms <- c("změna klima\\b", "\\bklima\\b\\s+se\\s+měnit\\b", "\\bměnící\\s+se\\s+klima", "\\bglobální oteplování", "\\boteplování planeta", "klimatický\\b(?= změna|podmínka|dopad|důsledek|model|opatření|krize|kolaps|katastrofa|neutrální|neutralita|plán|rozvrat|zákon|závazek|žaloba|vzdělávání)")

get_climate_term_counts <- function(chunk_path) {
  message("Processing chunk: ", chunk_path)
  
  data <- readRDS(chunk_path)
  
  matches <- sapply(climate_terms, function(term) {
    sapply(tolower(data$lemma), function(lemma) {
      str_detect(lemma, term)
    })
  })

  # Filter the data based on the matches
  data <- data[any(matches), ]

 
  # If the word is not present, print a message and return an empty data frame with the correct columns
  if (nrow(data) == 0) {
    message("No climate terms found in chunk: ", chunk_path)
    return(data.frame(doc_id = integer(), lemma = character(), year = integer(), climate_terms_count = integer()))
  }

  # Join with doc_id_by_date to get the date of each occurrence
  data <- data %>%
    left_join(doc_id_by_date, by = "doc_id") %>%
    mutate(year = as.character(year(date)))

  # Count the occurrences of each term
  data <- data %>%
    group_by(doc_id, year) %>%
    summarize(climate_terms_count = n()) %>%
    ungroup()

  return(data)
}
# Apply function to each file
climate_term_counts_list <- if (Sys.info()[['sysname']] == "Windows") {
  lapply(list_of_processed_chunks, get_climate_term_counts)
} else {
  mclapply(list_of_processed_chunks, get_climate_term_counts, mc.cores = detectCores() - 2)
}

# Bind results together into one data frame
climate_term_counts_df <- bind_rows(climate_term_counts_list)
```
...
if (Sys.info()[['sysname']] == "Windows") {
  # Use normal apply with Windows
  climate_term_counts_df <- lapply(list_of_processed_chunks, get_climate_term_counts) %>% bind_rows()
} else {
  climate_term_counts_df <- mclapply(list_of_processed_chunks, get_climate_term_counts, mc.cores = detectCores() - 2) %>% bind_rows()
}

saveRDS(climate_term_counts_df, "data/climate_term_counts_df.rds")

# Summarize the key term count per year
climate_term_counts_per_year <- climate_term_counts_df %>%
  group_by(year) %>%
  summarise(n_climate_terms = sum(climate_terms_count)) %>%
  ungroup() %>%
  # Add a row with the total counts for each term across all years
...
```
```{r Visualize all of the terms and their frequencies, NB see frequencies_new.Rmd}
# Install devtools if not already installed
if (!require(devtools)) {
  install.packages("devtools")
}

# Install bbcstyle from GitHub
...
# Create plot
plot_counts_terms <- term_counts_df %>%
  ggplot(aes(x = term, y = count / 1000)) +
  geom_col(fill = "#009087") +
  coord_flip() +
  ylab(element_blank()) +
  xlab(element_blank()) +
  scale_y_continuous(
    breaks = seq(0, max(term_counts_df$count) / 1000, max(term_counts_df$count) / 50000),
    labels = seq(0, max(term_counts_df$count) / 1000, max(term_counts_df$count) / 50000)
  ) +
  labs(
    title = "Decade of Czech Television news",
    subtitle = "Most frequent climate term occurrences in thousands"
  ) +
  bbc_style() +
  theme(
...
finalise_plot(plot_counts_terms, source_name = paste0("Source: Newton Media Archive, January 2012-April 2022. N=", sum(term_counts_df$count)), save_filepath = "visuals/term_counts.png", height_pixels = 850)
